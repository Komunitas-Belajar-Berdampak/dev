...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    if (!isAcceptedFileType(file, options.accept)) {
      const error = new Error('Only supported file types are allowed');
...
const ImageUploadDragArea: React.FC<ImageUploadDragAreaProps> = ({ onFile, children }) => {
  ...
  return (
    <div className={`tiptap-image-upload-drag-area ${isDragActive ? 'drag-active' : ''} ${isDragOver ? 'drag-over' : ''}`} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}>
      {children}
    </div>
  );
};
...
const ImageUploadPreview: React.FC<ImageUploadPreviewProps> = ({ fileItem, onRemove }) => {
  ...
  return (
    <div className='tiptap-image-upload-preview'>
      {fileItem.status === 'uploading' && <progress className='tiptap-image-upload-progress' value={fileItem.progress} max={100} />}

      <div className='tiptap-image-upload-preview-content'>
        <div className='tiptap-image-upload-file-info'>
          {fileItem.previewUrl && <img className='tiptap-image-upload-thumbnail' src={fileItem.previewUrl} alt={fileItem.file.name} draggable={false} />}
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  return (
    <NodeViewWrapper className='tiptap-image-upload' tabIndex={0} onClick={handleClick}>
      ...
      <input ref={inputRef} name='file' accept={accept} type='file' multiple={limit > 1} onChange={handleChange} onClick={(e: React.MouseEvent<HTMLInputElement>) => e.stopPropagation()} aria-label='Upload image' />
    </NodeViewWrapper>
  );
};
