  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success("Pengajar Berhasil Ditambahkan!", {
        description:
          selectedIds.length === 1
            ? "Pengajar berhasil ditambahkan sebagai pengajar."
            : `${selectedIds.length} pengajar berhasil ditambahkan sebagai pengajar.`,
      ...
      if (
        ...
        description = "Salah satu pengajar yang dipilih sudah menjadi pengajar di matakuliah ini.";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
      <DialogContent className="max-w-3xl w-[calc(100%-2rem)] sm:w-full rounded-xl">
        ...
        <div className="mt-4 relative" ref={containerRef}>
          ...
          >
            <span className={selectedIds.length > 0 ? "text-foreground truncate" : "text-muted-foreground"}>
              {selectedIds.length > 0 ? (selectedLabel || `${selectedIds.length} dipilih`) : "Pilih Dosen"}
            </span>
            <Icon
        ...
        >
          {submitting ? "Menyimpan..." : `Tambah Pengajar (${selectedIds.length})`}
        </Button>
      </DialogContent>
    </Dialog>
  );
