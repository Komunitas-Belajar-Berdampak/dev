...
const NotificationItem = ({ notification, variant = 'full', onClick, onDelete }: NotificationItemProps) => {
  ...
  return (
    ...
    >
      ...
      {!notification.isRead && <span className='absolute left-1 top-1/2 size-1.5 -translate-y-1/2 rounded-full bg-primary' aria-hidden />}
      ...
      <div className='min-w-0 flex-1'>
        <div className='flex items-center gap-2'>
          <p className={cn('truncate text-sm', notification.isRead ? 'font-medium text-foreground/80' : 'font-semibold text-foreground')}>{notification.judul}</p>
          {!isCompact && (
        ...
        <p className={cn('mt-0.5 text-xs text-foreground/70', isCompact ? 'line-clamp-2' : 'line-clamp-3')}>{notification.pesan}</p>

        <div className='mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-foreground/50'>
          {notification.course && <span className='font-medium text-foreground/60'>{notification.course.kodeMatkul}</span>}
