...
const StudyGroupMainContent = () => {
  ...
  const {
    ...
    queryFn: () => getCourses({ page: page, limit: 10 }),
  });
  ...
  useEffect(() => {
    if (!isError) return;
    toast.error(error?.message || "Gagal mengambil data mata kuliah.", {
      toasterId: "global",
    });
  }, [error?.message, isError]);

  const visibleCourses = useMemo(
    () => getVisibleCourses(courses, search, sort),
    [courses, search, sort],
  );
  ...
  return (
    <>
      ...
      {!isLoading && isError && (
        <ErrorMessage message="Tidak dapat memuat data." />
      )}
      {!isLoading && !isError && visibleCourses.length === 0 && (
        <NoData message="Tidak ada mata kuliah yang sesuai dengan pencarian." />
      )}
      {!isLoading && !isError && visibleCourses.length > 0 && (
        <StudyGroupList courses={visibleCourses} />
      )}
