...
export default function ChangePasswordModal({ open, nrp, userName, onSuccess }: Props) {
  ...
  const strength = useMemo(
    () => REQUIREMENTS.filter((r) => r.test(newPass)).length,
    [newPass],
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg: string =
        err?.response?.data?.message ?? err?.message ?? "Terjadi kesalahan saat mengubah password.";
      ...
      toast.error(title, {
        description, icon: toastIconError, style: styleError, descriptionClassName: "!text-white/90",
      });
    }
  };
      ...
      <DialogContent className="max-w-sm p-0 overflow-hidden rounded-2xl gap-0 [&>button]:hidden">
        ...
        <div className="px-8 pt-8 pb-5 text-center border-b">
          ...
          <p className="text-sm text-gray-500 mt-1 leading-relaxed">
            Halo{userName ? `, ${userName}` : ""}! Demi keamanan akun,
            harap ganti password default kamu sekarang.
          </p>
        </div>
        ...
        <div className="px-8 py-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input
                ...
                placeholder="Minimal kuat (3/4 kriteria)"
                className="pr-10"
            ...
            {newPass && (
              <div className="space-y-1 pt-0.5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {strength < 3 && (
                    <p className="text-xs text-gray-400">Minimal harus <span className="font-semibold text-blue-600">Kuat</span></p>
                  )}
                </div>
              </div>
            )}
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  <Icon
                    ...
                    className={`text-sm shrink-0 transition-colors ${ok ? "text-emerald-500" : "text-gray-300"}`}
                  />
          ...
          <Button
            ...
            disabled={!canSubmit}
            className="w-full border-2 border-black shadow-[3px_3px_0_0_#000] gap-2 h-11 font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading
              ? <><Icon icon="mdi:loading" className="animate-spin" />Menyimpan...</>
              : <><Icon icon="mdi:lock-check-outline" />Simpan Password Baru</>
            }
          </Button>
        </div>
      </DialogContent>
