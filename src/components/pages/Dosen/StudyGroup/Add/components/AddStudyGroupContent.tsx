...
const AddStudyGroupContent = ({ idMatkul }: AddStudyGroupContentProps) => {
  ...
  const nrpItems = useMemo(() => course?.mahasiswa.map((m) => m.nrp) ?? [], [course?.mahasiswa]);
  const namaByNrp = useMemo(() => new Map(course?.mahasiswa.map((m) => [m.nrp, m.nama])), [course?.mahasiswa]);
