...
function SkeletonBlock({ className = '' }: { className?: string }) {
  return <div className={['animate-pulse rounded-md bg-gray-200/80', className].join(' ')} />;
}
...
export default function DosenCourses() {
  ...
  const filteredMatakuliah = useMemo(() => {
    ...
    return list.filter((matkul) => {
      ...
      if (k) {
        if (!norm(matkul.kelas).includes(k)) return false;
      }
      if (p) {
        if (!norm(matkul.periode).includes(p)) return false;
      }
      if (st) {
        if (norm(matkul.status) !== st) return false;
      }
  ...
  const hasActiveFilter = search.trim() || kelas.trim() || periode.trim() || status.trim() || sks.trim();
      ...
      <div className='w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8'>
        ...
        <div className='flex flex-wrap items-center justify-between gap-4'>
          <div className='flex w-full sm:w-auto gap-2'>
            <Input placeholder='Search...' value={search} onChange={(e) => setSearch(e.target.value)} className='w-full sm:w-64 border border-black/20 text-blue-800 placeholder:text-gray-400' />
            <Button size='icon' className='border-2 border-black shadow-[3px_3px_0_0_#000]' type='button'>
              ...
              <div className='absolute left-0 mt-3 w-60 rounded-xl border-2 border-black bg-white p-5 shadow-[4px_4px_0_#000] space-y-4 z-50'>
                ...
                <div className='grid gap-3'>
                  <div className='h-10 rounded-md border border-black/20 bg-white px-3'>
                    <select value={status} onChange={(e) => setStatus(e.target.value as any)} className='h-full w-full bg-transparent text-sm outline-none text-gray-700'>
                ...
                <div className='flex items-center justify-between pt-1'>
                  <p className='text-xs text-gray-500'>{hasActiveFilter ? `${filteredMatakuliah.length} hasil` : ''}</p>
                  <div className='flex items-center gap-2'>
                    <Button variant='outline' className='border-2 border-black shadow-[3px_3px_0_0_#000]' type='button' onClick={resetFilters}>
                      Reset
                    </Button>
                    <Button className='bg-blue-900 text-white shadow-[3px_3px_0_0_#000]' type='button' onClick={() => setShowFilter(false)}>
                      Apply
                    </Button>
                  </div>
                </div>
        ...
        {!isLoading && error && (
          <div className='rounded-xl border-2 border-black bg-white p-6 shadow-[4px_4px_0_#000] space-y-3'>
            ...
            <Button onClick={() => void refetch()} className='bg-blue-900 text-white shadow-[3px_3px_0_0_#000]' type='button'>
              Coba lagi
            </Button>
          </div>
        )}
        ...
        {!isLoading && !error && filteredMatakuliah.length > 0 && (
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 py-8'>
            {filteredMatakuliah.map((matkul) => (
              <div key={matkul.id} className='flex flex-col rounded-2xl border-2 border-black bg-white shadow-[5px_5px_0_0_#000] overflow-hidden'>
                ...
                <div className='flex-1 p-4 space-y-2'>
                  ...
                  <p className='text-xs text-gray-600 line-clamp-3'>{matkul.deskripsi || '-'}</p>
                ...
                <div className='p-4 pt-0'>
                  <Button onClick={() => navigate(`courses/${matkul.id}`)} className='w-full bg-blue-900 text-white shadow-[3px_3px_0_0_#000] hover:translate-x-0.5 hover:translate-y-0.5 transition' type='button'>
                    Continue
                  </Button>
                </div>
              </div>
        ...
        {!isLoading && !error && filteredMatakuliah.length === 0 && (
          <div className='mt-16 flex flex-col items-center justify-center text-center'>
            ...
            {hasActiveFilter && (
              <Button onClick={resetFilters} className='mt-6 bg-blue-900 text-white shadow-[3px_3px_0_0_#000]' type='button'>
                Reset Filter
              </Button>
            )}
          </div>
