...
export default function DosenCourses() {
  ...
  const filteredMatakuliah = useMemo(() => {
    ...
    return list.filter((matkul) => {
      ...
      if (k)  { if (!norm(matkul.kelas).includes(k))   return false; }
      if (p)  { if (!norm(matkul.periode).includes(p))  return false; }
      if (st) { if (norm(matkul.status) !== st)          return false; }
  ...
  const hasActiveFilter =
    search.trim() || kelas.trim() || periode.trim() || status.trim() || sks.trim();

  function resetFilters() {
    setSearch(""); setKelas(""); setPeriode(""); setStatus(""); setSks("");
    setShowFilter(false);
  }
                ...
                <div className="flex-1 p-4 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-3 text-[11px] text-gray-500">
                    <span className="flex items-center gap-1">
                      <Icon icon="mdi:book-outline" />{matkul.sks} SKS
                    </span>
                    <span className="flex items-center gap-1">
                      <Icon icon="mdi:account-group-outline" />Kelas {matkul.kelas}
                    </span>
                    <span className="flex items-center gap-1">
                      <Icon icon="mdi:school-outline" />{matkul.periode ?? "-"}
                    </span>
                  </div>
